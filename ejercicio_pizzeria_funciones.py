...
def registrar_pizza():
    ...
    tipo_masa_nuevo = input("Ingrese el tipo de masa (delgada o tradicional): ")
    if tipo_masa_nuevo not in ['delgada', 'tradicional']:
        print(" error, masa no valida, Usando 'tradicional' ")
...
def ver_catalogo():
    print("\n--- Catálogo de Pizzas ---")
    if not pizzas:
        print("No hay pizzas registradas en el catálogo aún. Agregue algunas para verlas aquí")
    else:
...
def realizar_pedido():
    print("\n--- Realizar Pedido ---")
    if not pizzas:
        print("Lo sentimos, no hay pizzas disponibles, Registre algunas primero")
        return
    ...
    if not pizza_encontrada:
        ...
        return 
    if pizza_encontrada['stock'] == 0:
        print(f"Lo sentimos, la pizza '{pizza_encontrada['nombre']}' no tiene stock disponible en este momento")
        return
    while True:
        try:
